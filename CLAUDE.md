...
- **Estado real de indexación (dato del usuario, 2026-06-25, comprobado manualmente en Search Console, no vía el script)**: 11 de 23 artículos publicados ya están indexados en Google. Quedan 12 sin indexar — aún no se ha ejecutado `indexingChecker.js` con credenciales reales para saber cuáles exactamente ni en qué estado (rastreada/descubierta/error). Próximo paso lógico cuando exista la cuenta de servicio: ejecutar el script para identificarlas e ir resolviendo manualmente las que lleven más tiempo atascadas
