...
- [ ] Dar de alta los secrets `PINTEREST_ACCESS_TOKEN` y `PINTEREST_BOARD_ID` en GitHub — `PINTEREST_ENABLED` ya está en `true` (2026-06-23) y el workflow ya los referencia, pero sin los secrets reales cada publicación fallará Pinterest en silencio (no llega a Discord, ver "Publicación en redes sociales")
...
- **Pinterest**: ✅ **activado 2026-06-23** — `const PINTEREST_ENABLED = true`. `publishPinterest()` lee `PINTEREST_ACCESS_TOKEN`/`PINTEREST_BOARD_ID` de `process.env`, igual que Instagram/Facebook (sin placeholders)
- Manejo de errores: si Instagram falla, se loguea y continúa con Facebook; si Facebook falla, se loguea igual; si **ambas** fallan, se notifica a Discord vía `DISCORD_WEBHOOK_URL` con el detalle de ambos errores. **Pinterest no participa en esa comprobación** — si falla, se loguea pero nunca dispara la notificación a Discord, ni siquiera si Instagram y Facebook funcionan bien. Tenerlo en cuenta sobre todo ahora que está activado pero los secrets probablemente no existen aún (ver nota siguiente)
...
- **PENDIENTE — dar de alta los secrets en GitHub**: `PINTEREST_ACCESS_TOKEN` y `PINTEREST_BOARD_ID` están referenciados en el workflow y el código ya está activo, pero a fecha de hoy (2026-06-23) no hay confirmación de que existan como secrets reales en GitHub (no se puede verificar el valor desde aquí, solo la presencia, y `node socialPublisher.js --test` en local los marca como ❌ porque tampoco están en `.env` local). Hasta que se den de alta, cada publicación automática intentará Pinterest y fallará con "PINTEREST_ACCESS_TOKEN o PINTEREST_BOARD_ID no configurados" — el error se loguea en los logs del workflow pero, por el punto anterior, **no llega a Discord**. Revisar manualmente los logs de la próxima ejecución (25/06) para confirmarlo, o dar de alta los secrets antes de esa fecha
- Secrets nuevos usados: `INSTAGRAM_ACCESS_TOKEN`, `INSTAGRAM_APP_ID`, `INSTAGRAM_APP_SECRET`, `FACEBOOK_PAGE_ACCESS_TOKEN`, `FACEBOOK_PAGE_ID`, `PINTEREST_ACCESS_TOKEN`, `PINTEREST_BOARD_ID` (los 5 primeros ya disponibles en GitHub; los 2 de Pinterest pendientes de confirmar)
