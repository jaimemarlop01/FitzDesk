...
node instagramImageGenerator.js --slug [slug]                 # genera/regenera la imagen de Instagram a mano (Puppeteer, añadido 2026-06-24)
```
...
- **Imagen de Instagram (2026-06-24)**: `instagramImageGenerator.js` usa **Puppeteer** (Chromium headless) para renderizar una plantilla HTML/CSS y capturarla como `[slug]-instagram.png` a 1080x1350 (formato vertical 4:5). Diseño: imagen del producto a pantalla completa, degradado oscuro inferior para legibilidad, logo "🐿️ FitzDesk" arriba a la izquierda, badge naranja con la categoría arriba a la derecha, título en blanco recortado a 2 líneas con `-webkit-line-clamp`, puntuación grande (mismo color por tramos que `ScoreBox.astro`: verde ≥9, naranja ≥7.5, rojo el resto) y precio en naranja junto a ella. La imagen original se embebe como `data:` URI (sin servir archivos por `file://`). Lanza Chromium con `--no-sandbox --disable-setuid-sandbox` (necesario en runners de GitHub Actions). Probado en local y en una simulación completa de checkout limpio + `npm ci --omit=dev`
- Ambos generadores se ejecutan **bajo demanda** desde `socialPublisher.js` (`ensureSocialImage()`) si la imagen no existe todavía — no hace falta ejecutarlos a mano en el flujo normal, solo para pruebas o regeneración manual: `node socialImageGenerator.js --slug [slug]` (Facebook) / `node instagramImageGenerator.js --slug [slug]` (Instagram)
...
| `socialPublisher.js` | Publica en Instagram y Facebook con captions generados por IA (Pinterest preparado, desactivado) |
| `socialImageGenerator.js` | Genera la imagen de Facebook (Sharp + SVG) a partir de la imagen del artículo |
| `instagramImageGenerator.js` | Genera la imagen de Instagram (Puppeteer, plantilla HTML con puntuación y precio) |
