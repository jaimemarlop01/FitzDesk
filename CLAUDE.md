...
- **2 bugs técnicos de SEO encontrados a partir de esa captura, 1 corregido y 1 pendiente de acción manual**:
  - **✅ CORREGIDO — canonical autorreferenciado**: `BaseLayout.astro` generaba `<link rel="canonical">` a partir de `Astro.url.pathname` tal cual, sin normalizar. El sitio enlaza internamente sin barra final (`src/lib/url.ts`), pero la build estática también responde con 200 en la versión con barra — así que cada variante se autodeclaraba canónica a sí misma, sin consolidar nunca la señal en una sola URL. Esto explica los duplicados vistos en Search Console (`/keychron-k2-v2` y `/keychron-k2-v2/` como páginas distintas, mismo patrón en lg-display y en `/categoria/comparativas`). Corregido normalizando `Astro.url.pathname` (quitando la barra final salvo en la raíz) antes de construir `canonicalURL` — verificado en el HTML generado: `https://fitzdesk.com/articulo/keychron-k2-v2` sin barra, consistente con los enlaces internos
