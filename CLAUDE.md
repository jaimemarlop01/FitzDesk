...
- **Cambios 2026-07-24 (21ª pasada — objetividad editorial)**:
  ...
  - **4 guías publicadas sincronizadas**: las notas mencionadas en el cuerpo de las guías se actualizaron para coincidir con las puntuaciones revisadas — `mejor-raton`: MX Anywhere 3S 8.7→8.6 (heading + tabla); `mejor-monitor`: BenQ GW2780 8.2→7.0, Dell S2722QC 8.7→8.6; `teclado-mecanico-vs-membrana`: MX Keys S 8.8→8.0, Keychron V1 8.9→8.8, K2 V2 8.8→8.5; `dolor-muneca`: Lift Vertical 8.4→8.2, MX Keys S 8.8→8.0.
  - **`fitzdesk-monitor/analyzer.js` — rúbrica en el prompt**: campo `puntuacion` ahora incluye la rúbrica completa y la instrucción de usar decimales solo para diferenciar dentro del tier. `fitzQuote` y sección `## 🐿️ Fitz recomienda` actualizados para exigir que la nota en texto coincida exactamente con el campo `puntuacion` del frontmatter.
- **Cambios 2026-07-21 (20ª pasada — párrafos de introducción + corrección factual)**:
  - **SpecsTable dentro del prose**: confirmado que la inyección JS (antes del primer H2 dentro de `.prose`) es el enfoque correcto. Mover la tabla fuera del prose causaba que la caja de actualización de precio (`> 📅 **Artículo actualizado...`), primer elemento del Markdown, apareciera visualmente por debajo de las specs — no encima del precio como se esperaba. Revertido a la inyección JS original.
  - **Párrafos de introducción en todos los análisis publicados**: 16 artículos carecían de texto antes del primer H2, lo que producía un salto inconsistente (en algunos artículos las specs aparecían inmediatamente, en otros había texto previo). Añadido un párrafo introductorio en los 16 para uniformizar el layout: `[aviso actualización] → [intro] → [SpecsTable] → [H2 secciones]`.
  - 2 artículos tenían un `## Introducción` H2 genérico como envoltorio innecesario — eliminado el H2, el texto del párrafo queda como intro directa: `asus-rog-strix-scar-18-analisis`, `lg-ultragear-34gx90sb-w-analisis`.
  - 14 artículos con párrafo de intro nuevo: `asus-vivobook-15-oled-analisis`, `benq-gw2780-analisis`, `dell-s2722qc-analisis`, `keychron-k2-v2`, `keychron-k8-pro-analisis`, `keychron-v1-analisis`, `lenovo-thinkpad-e14-gen6-analisis`, `lg-27un880`, `lg-27up850n-analisis`, `lg-gram-14-2025-analisis`, `logitech-lift-vertical-analisis`, `logitech-mx-anywhere-3s-analisis`, `logitech-mx-keys-s-analisis`, `logitech-mx-master-3s-analisis`.
  - **2 correcciones factuales en intros**: `lg-gram-14`: "más de 18 horas de batería" → "hasta 18 horas de batería declarada" (el artículo dice "hasta 18 horas"). `logitech-mx-anywhere-3s`: "soporte para tres dispositivos" → "soporte para dos dispositivos (Logi Bolt y Bluetooth)" (el MX Anywhere 3S solo soporta 2, no 3 como el MX Master 3S).
  - **Corrección factual Dell S2722QC**: la 19ª pasada cambió `keyword_principal` a `"monitor QHD USB-C Dell"` partiendo de la suposición errónea de que el Dell es QHD. Verificado por el usuario: el Dell S2722QC es 4K (3840×2160). `keyword_principal` corregido a `"monitor 4K USB-C Dell"` (diferencia de marca respecto al LG 27UP850N-W, que conserva `"monitor 4K USB-C LG"`). El cuerpo del artículo ya era correcto (specs, resolución y texto en 4K); solo el keyword_principal y el intro (que se escribió como QHD en esta misma sesión antes de verificar) necesitaban corrección.
  - **Estilos de tabla en el prose**: `global.css` no tenía ninguna regla para `table` dentro de `.prose` — el navegador los renderizaba sin padding, lo que hacía que las columnas se pegaran. Añadidos estilos completos: `border-collapse`, padding de celdas (0.65rem 1rem), línea divisoria entre filas (`border-bottom`), cabecera con `font-weight: 600` y fila activa con `hover` sutil.
  - **Tablas de guías publicadas**: las dos guías de setup publicadas (`mejor-setup-teletrabajo-500-euros-2026`, `setup-teletrabajo-profesional-2026`) tenían en la tabla "Resumen del setup" enlaces a PcComponentes que no funcionaban. Sustituidos por enlaces internos a los análisis propios de FitzDesk ("Ver análisis →") y renombrada la columna de "Enlace" a "Análisis".
  - **Pill "Ofertas" eliminada de `/analisis`**: el filtro `🔥 Ofertas` quitado de las pills de categoría de `analisis.astro` — no hay artículos de oferta publicados y el botón no aportaba nada. La lógica de filtrado interna se conserva para cuando sea necesario.
- **Cambios 2026-07-20 (19ª pasada — SEO + contenido)**:
  ...
  - **SEO — keyword Dell**: `dell-s2722qc-analisis` `keyword_principal` diferenciado a `"monitor 4K USB-C Dell"` para no canibalizar al LG 27UP850N-W (`"monitor 4K USB-C LG"`). ⚠️ La nota original de esta pasada decía erróneamente que el Dell S2722QC es QHD (2560×1440) — **incorrecto; verificado por el usuario en la 20ª pasada: el Dell S2722QC es 4K (3840×2160)**. El keyword quedó como "monitor QHD USB-C Dell" hasta la 20ª pasada, donde se revirtió a "monitor 4K USB-C Dell"
...
- **Bug de timeouts CDN corregido 2026-07-28**: el paso "Disparar deploy" tenía timeout de 5 min y "Esperar imágenes" solo daba 5.5 min de polling — total ~10.5 min insuficiente con retrasos de cola de GitHub Actions. Resultado: imágenes con 404 al sondear → `esperar_imagenes.outcome == 'failure'` → Instagram saltado sin error visible (Discord enviaba "⚠️ Carrusel no disponible — publicar redes manualmente"). LG Ultragear (21/07) y Jabra (23/07) afectados, publicados manualmente en Instagram el 28/07. **Fix**: `publicar-automatico.yml` y `publicar-en-redes.yml` — timeout-minutes 5→12 en "Disparar deploy", timeout-minutes 6→12 en "Esperar imágenes", MAX_WAIT 300→600 (commit `6d1b5a8` en develop)
